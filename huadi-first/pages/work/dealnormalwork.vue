<template>
  <div class="block">
    <el-form :inline="true" :model="userquery" class="demo-form-inline">
      ...
      <el-select v-model="userquery.sex" placeholder="请选择性别">
...
export default {
  ...
  methods: {
    ...
    pass(id) {
      userApi.dealNormalApply(id, true).then((response) => {
        this.$message.success(response.message);
      });
       setTimeout(function () {
              window.location.href = "/work/dealnormalwork";//1秒后刷新页面
            }, "1000");
    },
    defeat(id) {
      userApi.dealNormalApply(id, false).then((response) => {
        this.$message.success(response.message);
      });
     setTimeout(function () {
              window.location.href = "/work/dealnormalwork";//1秒后刷新页面
    ...
    getInfo(page = 1) {
      ...
      userApi
        .getNormalDealInfo(this.page, this.size, this.userquery)
        .then((response) => {
